...
# Optionally, print a few entries to see the content
print("First few entries in the dataset:\n", dataset[:5])

# If it's a multidimensional array, you can also explore specific dimensions
# For example, if it's an image dataset, you might want to visualize an image
import matplotlib.pyplot as plt
...
model_filename = "Transformer-NAO-Lorenz"
...
if dataset.ndim == 4:  # Example for image datasets with shape (num_samples, height, width, channels)
...
learning_rate = 1e-2  # List of learning rates to try
gamma = 0.7  # List of gamma values (learning rate decay) to try
wd = 1e-5  # List of weight decay values to try

# Model parameters
nb = 3  # Number of layers in the model
dk = 10  # Dimension of the key/query vectors
...
S = 3  # Spatial resolution (e.g., 21x21 grid)
dt = 5e-5  # Time step size
sall = 10  # Feature dimension (same as dk here)


# Dataset splitting
ntotal = 100  # Total number of samples
ntrain = 90  # Number of training samples
ntest = 10  # Number of test samples

# Time series configuration
n_timesteps = 1000  # Total number of time steps
...
out_dim = S  # Output dimension (e.g., 21x21 grid)
...
sol_prev_train = torch.tensor(np.load('lorenz_soln.npy'))[train_list, :].view(ntrain, sample_per_task, n_timesteps, S)
y_train = torch.tensor(np.load('lorenz_load.npy'))[train_list, :].view(ntrain, sample_per_task, n_timesteps, S)

sol_prev_test = torch.tensor(np.load('lorenz_soln.npy'))[test_list, :].view(ntest, sample_per_task, n_timesteps, S)
y_test = torch.tensor(np.load('lorenz_load.npy'))[test_list, :].view(ntest, sample_per_task, n_timesteps, S)
...
sol_train_u0 = sol_train_u0[:ntrain, ...].reshape(ntrain, nt, S).permute(0, 2, 1).float() #ntrain X 3 X 995
sol_train_u1 = sol_train_u1[:ntrain, ...].reshape(ntrain, nt, S).permute(0, 2, 1).float()
sol_train_u2 = sol_train_u2[:ntrain, ...].reshape(ntrain, nt, S).permute(0, 2, 1).float()
sol_train_u3 = sol_train_u3[:ntrain, ...].reshape(ntrain, nt, S).permute(0, 2, 1).float()
sol_train_u4 = sol_train_u4[:ntrain, ...].reshape(ntrain, nt, S).permute(0, 2, 1).float()
sol_train_u5 = sol_train_u5[:ntrain, ...].reshape(ntrain, nt, S).permute(0, 2, 1).float()
sol_train_y = sol_train_y[:ntrain, ...].reshape(ntrain, nt, S).permute(0, 2, 1).float()
...
u0_train = torch.cat(u0_df, dim=0) # ntrain*n_randperm X 3 X sall
...
# Concatenate normalized tensors incrementally to form the input sequence
#u_seq1 = u0_train
#for u_train in tqdm([u1_train, u2_train, u3_train, u4_train]):
#    u_seq1 = torch.cat((u_seq1, u_train), dim=1) # ntrain*n_randperm X 15 X sall
...
print("Transformer Input dimension:", input_dim, "Output dimension:", output_dim, "Number of previous timesteps:", int(input_dim / output_dim))
...
sol_test_u0 = sol_test_u0[:ntest, ...].reshape(ntest, nt, S).permute(0, 2, 1).float()
sol_test_u1 = sol_test_u1[:ntest, ...].reshape(ntest, nt, S).permute(0, 2, 1).float()
sol_test_u2 = sol_test_u2[:ntest, ...].reshape(ntest, nt, S).permute(0, 2, 1).float()
sol_test_u3 = sol_test_u3[:ntest, ...].reshape(ntest, nt, S).permute(0, 2, 1).float()
sol_test_u4 = sol_test_u4[:ntest, ...].reshape(ntest, nt, S).permute(0, 2, 1).float()
sol_test_u5 = sol_test_u5[:ntest, ...].reshape(ntest, nt, S).permute(0, 2, 1).float()
sol_test_y = sol_test_y[:ntest, ...].reshape(ntest, nt, S).permute(0, 2, 1).float()
...
embed_dim = 20  # Embedding dimension for transformer
...
tokend = u0_train.shape[1]  # This is an add-on to the neural operator, =S=3
